function process_filter_optimization(processing_config::PropDict, l200::LegendData, period::DataPeriod, run::DataRun,; reprocess::Bool=false, timeout::Int=0)
    ...
    function ch_filter_optimization(chinfo_ch::NamedTuple)
        ...
        # get QC cuts (only if ML model is available)
        blmean_wdw = nothing
        if ml_available
            try
                @debug "Get QC cuts"
            ...
            @debug "Skipping QC cuts - using all $(length(wvfs_ch_pre)) waveforms (100% survival fraction)"
            # Set blmean_wdw to default values when no QC is applied
            blmean_wdw = zeros(length(wvfs_ch_pre))
        end
