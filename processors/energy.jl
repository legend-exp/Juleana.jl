        ...
        if !reprocess && haskey(pars_db, det)
            ...
            for e_type in energy_types
                if haskey(pars_db[det], e_type)
                    log_info = log_nt((ch, det, ProcessStatus(1), e_type, pars_db[det][e_type].fwhm_qbb, pars_db[det][e_type].Tl208FEP.fwhm, pars_db[det][e_type].m_calib, "Already processed --> skipped."))
                    processed_dict[e_type] = false
                    log_info_dict[e_type] = log_info
                end
                if haskey(pars_db[det], Symbol("$(e_type)_ctc"))
                    log_info = log_nt((ch, det, ProcessStatus(1), Symbol("$(e_type)_ctc"), pars_db[det][Symbol("$(e_type)_ctc")].fwhm_qbb, pars_db[det][Symbol("$(e_type)_ctc")].Tl208FEP.fwhm, pars_db[det][Symbol("$(e_type)_ctc")].m_calib, "Already processed --> skipped."))
                    processed_dict[Symbol("$(e_type)_ctc")] = false
