function process_energy_calibration_tails(processing_config::PropDict, l200::LegendData, period::DataPeriod, run::DataRun,; reprocess::Bool=false, timeout::Union{Int, Bool}=false,  dependencies::Vector{Any})
    ...
    mkpath(joinpath(data_path(l200.par.rpars.ecal_tails), string(period)))
    pars_db = PropDict(l200.par.rpars.ecal_tails[period, run])
        ...
        @showprogress desc="Detector: $det" for e_type in energy_types
            ...
            try
                ...
                try
                    ...
                    result_fit, report_fit = fit_peaks(result_simple.peakhists, result_simple.peakstats, th228_names; e_unit=result_simple.unit, calib_type=:th228, fit_func = :f_fit_tails)
    ...
    writelprops(l200.par.rpars.ecal_tails[period], run, pars_db)
    # writevalidity(l200.par.rpars.ecal_tails, filekey, (period, run))
