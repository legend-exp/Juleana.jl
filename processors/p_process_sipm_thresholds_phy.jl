function p_process_sipm_thresholds_phy(processing_config::PropDict, l200::LegendData, period::DataPeriod,; reprocess::Bool=false, timeout::Int=0, only_first_period::Bool=true)
        
    @info "Process SiPM thresholds for all partitions containing period $period"
    ...
    log_nt = NamedTuple{(:Channel, :Detector, :Partition, :Status, Symbol("σ Trigger"), Symbol("σ DC Trigger"), :Error)}
    ...
    function ch_sipm_threshold(chinfo_ch::NamedTuple)
        ...
        pars_db_ch = if isfile(joinpath(data_path(l200.par.ppars.sipmthres), "$det", "$part.json"))
            PropDict(l200.par.ppars.sipmthres[det, part])
        else
            PropDict()
        end
