        ...
        try
            if !all([haskey(processed_dict, aoe_classifier) for aoe_classifier in aoe_classifiers])
                hit_cal = fast_flatten([begin
                    ...
                    calibrate_ged_detector_data(l200, pinfo.cal.startkey, det, read_ldata(:dataQC, l200, :jlhit, :cal, pinfo.period, pinfo.run, det); aoe_cal_pars_type=:rpars, aoe_cal_pars_cat=:aoe, aoe_cut_pars_type=:rpars, aoe_cut_pars_cat=:aoe) end
