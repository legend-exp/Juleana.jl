    ...
    function ch_aoe_cut(chinfo_ch::NamedTuple)
        ...
        @showprogress desc="Detector: $det" for aoe_type in aoe_types
            ...
            try
                ...
                log_info = log_nt_cal(ch, det, ProcessStatus(1), aoe_type, length(compton_bands), mean(result_correction.µ_compton.gof.residuals_norm), mean(result_correction.σ_compton.gof.residuals_norm), "-")
        ...
        @showprogress desc="Detector: $det" for aoe_classifier in aoe_classifiers
            ...
            try
                ...
                plot!(plot_title=get_plottitle(filekey, det, "A/E Performance"; additiional_type=string(aoe_classifier)), plot_titlelocation=(0.5,0.2), plot_titlefontsize = 9)
                savelfig(savefig, p, l200, filekey, det, Symbol("aoe_peaks_sf_$aoe_classifier"))
                ...
                plot!(plot_title=get_plottitle(filekey, det, "A/E Performance"; additiional_type=string(aoe_classifier)), plot_titlefontsize = 12)
                savelfig(savefig, p, l200, filekey, det, Symbol("aoe_qbb_sf_$aoe_classifier"))
