...
All three sit at the event level (length `n_evts`) and combine two granularities: scalar event-level cuts/sums on one side, per-SPM-fiber VoVs on the other (where applicable).

#### `ged_spm/` and `ft_spm/` (identical schema)

Two filter families: `trig_max_cal_*` and `trig_max_trap_cal_*`. Each family has the same shape, so only the `trig_max_cal_*` block is shown below.

**Event-scalar cuts and cross-fiber sums** (one value per event, summed *over all SPM fibers* in the relevant time window):

```
is_valid_lar :: Bool                                LAr-veto pass/fail
trig_max_cal_lar_cut :: Bool                        cut Bool
trig_max_cal_spms_win_pe_sum :: Quantity{Float64}   sum over all fibers in full window (-1 .. 5 µs)
trig_max_cal_spms_win_multiplicity :: Int64         count of fibers above pe_trig_threshold in full window
```

**Per-SPM-fiber VoVs** (one value per fiber per event, DC triggers excluded, threshold applied — three windows):

```
trig_max_cal_spms_pe_sum :: VoV{Quantity{Float64}}          full window     (-1 .. 5 µs)
trig_max_cal_spms_pe_sum_prompt :: VoV{Quantity{Float64}}   prompt window   (-1 ..  1 µs)
trig_max_cal_spms_pe_sum_delayed :: VoV{Quantity{Float64}}  delayed window  ( 1 ..  5 µs)
```

Per event, the inner vector of these VoVs has length = number of SPM fibers in that event, and uses **the same fiber order as `/jlevt/spms/detector`** for the same event. The window cuts come from the `evt` config (`ged_sum_window`, `ged_sum_window_prompt`, `ged_sum_window_delayed`).

Identities (per event, per fiber `i`):

- `pe_sum[i]` = sum of pe of (non-DC, above-threshold) triggers of fiber `i` in the full window
- `pe_sum_prompt[i] + pe_sum_delayed[i] ≈ pe_sum[i]` (windows decompose the full window)
- `sum(pe_sum[*])` over all fibers ≈ `win_pe_sum` (cross-fiber sum)

**How to read S-specific values**: the per-fiber VoVs share the SPM-detector axis with `/jlevt/spms/detector`, so `read_ldata(_, l200, (:jlevt, fk, det))` for an SPM detector auto-indexes them at the SPM's per-event slot — you get a `Vector{Quantity}` per requested column, one scalar per event for that SPM.

The `trig_max_trap_cal_*` family mirrors this structure (alternative DSP filter).

**Note**: there is *no* `_win_pe_sum_prompt` / `_win_pe_sum_delayed` in the file — the cross-fiber sum is only stored for the full window. For prompt/delayed cross-fiber sums use `sum.(pe_sum_prompt)` / `sum.(pe_sum_delayed)`.

#### `ged_pmt/` (5 columns, all event-scalar)

```
is_valid_muon :: Bool
multiplicity :: Int64
n_hit_window :: Int64
pe_cal :: Quantity{Float64}
timestamp :: Quantity{Float64}
```
