...
result_blmean, report_blmean = get_centered_gaussian_window_cut(data_ch.blmean, qc_config.blmean.min, qc_config.blmean.max, 2,; n_bins_cut=convert(Int64, round(length(data_ch)/100)), relative_cut=0.01, fixed_center=false, left=true)
plot(report_blmean, xlabel="Baseline Mean (ADC)", ylabel="Counts")
plot!(title=format("{} Baseline Mean ({}-{}-{}-{})", string(det), string(filekey.setup), string(filekey.period), string(filekey.run), string(filekey.category)))

result_blsigma, report_blsigma = get_centered_gaussian_window_cut(data_ch.blsigma, qc_config.blstd.min, qc_config.blstd.max, 4,; n_bins_cut=convert(Int64, round(length(data_ch)/100)), relative_cut=0.01, fixed_center=false, left=true)
plot(report_blsigma, xlabel="Baseline Std (ADC)", ylabel="Counts")

histogram(data_ch.blsigma, bins=0:0.01:20)
...
result_blslope_cut, report_blslope_cut = get_centered_gaussian_window_cut(data_ch.blslope, qc_config.blslope.min*u"ns^-1", qc_config.blslope.max*u"ns^-1", 2,; n_bins_cut=convert(Int64, round(length(data_ch)/20)), relative_cut=qc_config.blslope.rel_cut)
plot(report_blslope_cut, xlabel="Baseline Slope (ns^-1)", ylabel="Counts")
plot!(title=format("{} Baseline Slope ({}-{}-{}-{})", string(det), string(filekey.setup), string(filekey.period), string(filekey.run), string(filekey.category)))
