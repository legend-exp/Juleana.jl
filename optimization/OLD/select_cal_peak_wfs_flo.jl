...
get_raw_ch_data(ds::LHDataStore, ch::Integer) = ds["ch$(ch)"].raw


function get_daqenergy(ds::LHDataStore, ch::String)
    # Should be (ToDo - make this faster):
    #   get_raw_ch_data(ds, ch).daqenergy[:]
    # Faster workaround, temporary:
    ds.data_store[ch]["raw"]["daqenergy"][:]
end

function get_all_daqenergies(filelist::AbstractVector{<:AbstractString})
    flatten_by_channel([
        LHDataStore(
            ds -> IdDict(ch => get_daqenergy(ds, ch) for ch in channel_list if ch in keys(ds)), filename
...
channels = parse.(Int, replace.(filter(startswith("ch"), channel_list), Ref("ch" => "")))
# testlist = String[string("/remote/ceph2/group/legendex/data/l200/raw/cal/p02/r006/l200-p02-r006-cal-20221226T193732Z-tier_raw.lh5"), string("/remote/ceph2/group/legendex/data/l200/raw/cal/p02/r006/l200-p02-r006-cal-20221226T193938Z-tier_raw.lh5")]
println("Get DAQ energies")
daq_energies = get_all_daqenergies(filelist)
println("Calibrate")
calib_funcs = IdDict(((k, auto_calib_func(v)) for (k,v) in daq_energies))

println("Filter peaks")
...
h5open(joinpath(folder_peaks, "peaks.lh5"), "w") do output
    for ch in channels
