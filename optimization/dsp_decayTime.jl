...
for ch in channel_list
    ...
    filename = joinpath(folder_dict["folder_peaks"], format("{}-p{:02d}-r{:03d}-cal-{}-tier_raw.lh5", experiment, period, calrun, ch))
    data_ch = LHDataStore(filename, "r")

    fep_ch = data_ch["$ch/Tl208dFEP"][:]
    ...
    opt_r = optimize(f_loglike , [hists.edges[1][cts_argmax], 10])
    μ, σ = opt_r.minimizer

    d = fit(Normal, uconvert.(NoUnits, τ_fit./1u"µs"))

    # histogram(τ_fit, bins=100, normalize=:pdf, xrange=(min_τ, max_τ), label="Decay Times")
    plot!(truncated(Normal(μ, σ), cut_low, cut_high)*sum(hists.weights), label=format("Decay Time Fit (µ = {:.2f}, σ = {:.2f})", μ, σ), color="red", line_width=3.5)
