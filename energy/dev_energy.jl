...
# @everywhere function ch_energy_calibration(i::Int64)
    ...
    i = findfirst(chinfo.detector .== :V02162B)
    ...
    if !reprocess && haskey(pars_db, det)
        ...
        for e_type in energy_types
            if haskey(pars_db[det], e_type)
                if haskey(pars_db[det][e_type], :energy)
                ...
                if haskey(pars_db[det][Symbol("$(e_type)_ctc")], :energy)
    ...
    # for e_type_name in energy_types
        e_type_name = :e_trap
        if haskey(result_dict, e_type_name)
            continue
        end
        
        # for e_type in [e_type_name, Symbol("$(e_type_name)_ctc")]
        e_type = Symbol("$(e_type_name)_ctc")
        # e_type = e_type_name
            # try
                ...
                plot(plot.(result_simple.peakhists, yscale=:log10)...)
                yield()
