...
function fitStrings(data_strings::Dict, string_numbers::Array, figure_folder::PosixPath, qc_cuts::Table)
    # check output figure folder
    energy_figure_folder = joinpath(figure_folder, "energy")
    checkFolder(energy_figure_folder, true)
    # Fit strings
    for string_number in string_numbers
        ...
        for (i, ch) in enumerate(channel_list)
            ...
            data_ch = dsp_data[ch]
            qc_ch = qc_cuts[findfirst(qc_cuts.channel .== ch):findlast(qc_cuts.channel .== ch)]

            if !(all(data_ch.timestamp .== qc_ch.timestamp))
                error("Timestamps do not match")
                continue
            end
            e_uncal = data_ch.e_10410[qc_ch.qc]
            fitChannel(e_uncal, ch, th228_lines, window_size, n_bins, string_energy_figure_folder, label_list[i], label_listExt[i])
        end
    end
end
