    ...
    if !reprocess && haskey(pars_db, det)
        ...
        for e_type in energy_types
            if haskey(pars_db[det], e_type) && haskey(pars_db[det][e_type], :ctc)
                log_info = "| $ch | $det | Success | $e_type | $(round(pars_db[det][e_type].fwhm_qbb, digits=2))±$(round(pars_db[det][e_type].fwhm_qbb_err, digits=2)) | $(round(pars_db[det][e_type][:Tl208FEP].fwhm, digits=2))±$(round(pars_db[det][e_type][:Tl208FEP].err.fwhm, digits=2)) | $(round(pars_db[det][e_type].m_calib, digits=2)) | Already processed --> skipped. |"
...
# fk_problem = FileKey("l200-p03-r000-cal-20230312T015420Z")
# rm(l200.tier[:dsp, fk_problem])
    # LHDataStore(l200.tier[:dsp, fk], "r")
for fk in filekeys
    @info string(fk)
    # if fk == fk_problem
    #     continue
    ...
    channel_not_in_ds = false
    LHDataStore(l200.tier[:dsp, fk], "r") do ds
        if !haskey(ds, ch)
            @warn "Channel $ch not found in $(basename(l200.tier[:dsp, fk])), skip"
            channel_not_in_ds = true
        end
    end
    if channel_not_in_ds
        continue
    end
    push!(ch_filekeys, fk)
end
...
data_ch = fast_flatten([
    ...
    # for e_type in energy_types
        ...
        # try
            ...
            plot(report_simple, margin=5mm, yformatter=:plain, thickness_scaling=1.5, cal=false, title=format("{} Simple Calibration ({}-{}-{}-{})", string(det), string(filekey.setup), string(filekey.period), string(filekey.run), string(filekey.category)))
            ...
            plot(report_ctc, size=(300, 200), plot_title=format("{} {}keV Charge Trapping Correction ({}-{}-{}-{})", string(det), string(ctc_cal_peak), string(filekey.setup), string(filekey.period), string(filekey.run), string(filekey.category)))
