...
plotlyjs()
result_simple, report_simple = simple_calibration(dsp_data.e_trap, th228_lines, window_sizes, n_bins=n_bins)
plot(report_simple, cal=true, size=(1400, 1000), title=format("{} Simple Calibration ({}-{}-{}-{})", string(det), string(filekey.setup), string(filekey.period), string(filekey.run), string(filekey.category)))
...
@debug "Fit all peaks"
result_peaks, report_peaks = fit_peaks(result_simple.peakhists, result_simple.peakstats, th228_lines)
v_ml, opt_r = fit_peaks_combined(result_simple.peakhists, result_simple.peakstats, th228_lines)
...
plot([begin
    ...
    plot!(minimum(h.edges[1]):0.1:maximum(h.edges[1]), Base.Fix2(LegendSpecFits.th228_combined_fit_functions.f_fit, v))
    plot!(minimum(h.edges[1]):0.1:maximum(h.edges[1]), Base.Fix2(LegendSpecFits.th228_combined_fit_functions.f_bck, v))
    plot!(minimum(h.edges[1]):0.1:maximum(h.edges[1]), Base.Fix2(LegendSpecFits.th228_combined_fit_functions.f_lowEtail, v))
end; for i in eachindex(result_simple.peakhists)]..., size=(1400, 1000))

plot(52:0.01:70.0, x ->  LegendSpecFits.ex_gauss_pdf(-x, -60.0, 1.0, 0.0))
plot!(52:0.01:70.0, x -> LegendSpecFits.ex_gauss_pdf(-x, -60.0, 1.0, 5.0))
plot!(52:0.01:70.0, x -> LegendSpecFits.ex_gauss_pdf(-x, -60.0, 1.0, 0.1))
...
μ = [result[p].μ for p in th228_lines]
μ_err = [result[p].err.μ for p in th228_lines]
