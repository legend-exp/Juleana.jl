    ...
    # create workers
    create_workers(processing_config, :filter_optimization)
    ...
    # move all variables to workers
        ...
        if length(keys(result_rt_dict)) == length(keys(optimization_config_ch)) - 1
        ...
        #     try
                ...
                try
                    ...
                    e_grid = getfield(Main, Symbol("dsp_$(filter_type)_ft_optimization"))(wvfs_ch_fep, dsp_config, pars_tau[det].tau, Measurements.value(result_rt.rt))
                catch e
                ...
                p = plot(report_ft)
                title!(p, get_plottitle(filekey, det, "FEP FT Scan"; additiional_type=string(filter_type)))
                ...
                log_info = log_nt((ch, det, ProcessStatus(-1), filter_type, "-", "-", "-", "$e"))
                # add results to dict
                log_info_dict[filter_type] = log_info
                processed_dict[filter_type] = false
            end
        ...
        # return results
        filter_types_processed = collect(keys(processed_dict))
        result = NamedTuple{Tuple(filter_types_processed)}([(ft = result_ft_dict[ft], rt = result_rt_dict[ft]) for ft in filter_types_processed]...)
        return (result = result, log = log_info_dict, processed = processed_dict)

    result_flt = parallel(chinfo, ch_filter_optimization, log_nt; timeout=timeout, retry=false)
