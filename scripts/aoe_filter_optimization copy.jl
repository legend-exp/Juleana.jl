...
append!(tbl, [res.log for (itr, res) in result_sg if !(res.log isa Dict)])
unique_keys = unique(reduce(vcat, collect.(keys.(tbl))))
lreport!(report, StructArray([NamedTuple{Tuple(unique_keys)}([get(nt, k, "-") for k in unique_keys]...) for nt in tbl]))
