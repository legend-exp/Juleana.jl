function process_dsp_phy(l200::LegendData, period::DataPeriod, run::DataRun,; reprocess::Bool = false, timeout::Int=3600)
    ...
    chinfo_raw = channel_info(l200, filekey)
    ...
    @everywhere function single_file_dsp(idx::Int64)
        ...
        @timeit dsp_timer "DSP" begin
            ...
            for det in keys(dsp_meta.additional_channel)
                det_row = filter(row -> row.detector == det, chinfo_raw)
                ch_short = det_row.channel[1]
                ...
                @timeit dsp_timer "DSP $det" begin
                    ...
                    outdata_ch = nothing
                    try
                        outdata_ch = getfield(Main, Symbol(dsp_meta.additional_channel[det]))(data_ch, dsp_config)
                        ch_sucess = true
                    catch e
